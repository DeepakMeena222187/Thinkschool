...
using QuotesApi.QueryTranslation;
...
if (args.Contains("--querydemo"))
{
    var queryDemoConnectionString = builder.Configuration.GetConnectionString("Quotes")
        ?? throw new InvalidOperationException("Missing required configuration value: ConnectionStrings:Quotes.");

    await QueryTranslationDemo.RunAsync(queryDemoConnectionString);
    return;
}

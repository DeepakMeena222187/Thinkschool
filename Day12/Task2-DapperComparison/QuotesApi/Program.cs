...
if (args.Contains("--dappercompare"))
{
    var benchmarkConnectionString = builder.Configuration.GetConnectionString("Quotes")
        ?? throw new InvalidOperationException("Missing required configuration value: ConnectionStrings:Quotes.");

    DapperComparisonBenchmark.Run(benchmarkConnectionString);
    return;
}
...
app.MapSlowEndpoints();
app.MapFastEndpoints();
app.MapCqrsQuoteEndpoints();

...
public static class CqrsQuoteEndpointExtensions
{
    ...
    public static void MapCqrsQuoteEndpoints(this WebApplication app)
    {
        ...
        // Same contract (GetQuoteListQuery's CurrentUserId in, List<QuoteListItemDto> out),
        // routed to GetQuoteListQueryDapperHandler instead of the EF Core handler above -
        // see Features/Quotes/GetQuoteListQueryDapperHandler.cs for why this needs its own
        // request type rather than a second handler for GetQuoteListQuery itself.
            ...
            CancellationToken ct) =>
        {
            ...
            var quotes = await mediator.Send(new GetQuoteListQueryDapper(resolvedUserId.Value), ct);

...
{
    [Fact]
    public void Valid_construction_sets_name_owner_and_starts_empty()
    {
        var collection = new Collection("Summer Reads", 42);

        collection.Name.Should().Be("Summer Reads");
        collection.OwnerId.Should().Be(42);
        collection.Items.Should().BeEmpty();
    }

    [Fact]
    public void Name_shorter_than_3_characters_throws()
    {
        Action act = () => new Collection("ab", 1);
    ...
    [Fact]
    public void Adding_the_first_item_stores_its_quote_id()
    {
        var collection = new Collection("Reading List", 1);

        collection.AddItem(7);

        collection.Items.Should().ContainSingle().Which.QuoteId.Should().Be(7);
    }
